...
{    
    ...
    {
        Utilities.EmbedColourText(Colour.ENHANCEMENT, Colour.ITEM, Colour.ITEM, "", "Enhance", " your ", "[W]","eapon or your ","[A]","rmor?\n\n");
        string choice = Console.ReadKey(true).KeyChar.ToString().ToLower();

        if (choice == "w")
        {
            Utilities.EmbedColourText(Colour.ENHANCEMENT, Colour.ITEM, "", "Enhance", " your ", $"{p.Weapon.name}", "?\n\n[Y]es        [N]o\n\n");
            string confirm = Console.ReadKey(true).KeyChar.ToString().ToLower();
            if (confirm =="y") ItemChange(p, p.Weapon);
        }
        if (choice == "a")
        {
            Utilities.EmbedColourText(Colour.ENHANCEMENT,Colour.ITEM,"","Enhance"," your ",$"{p.Armor.name}","?\n\n[Y]es        [N]o\n\n");
            string confirm = Console.ReadKey(true).KeyChar.ToString().ToLower();
    ...
    private static void ItemChange(Creature p, Equipment item)
    {
        ...
        if (choice > 0 && choice < CraftMachineEnhancements.Count)
        {
            if (IngredientCheck(p, CraftMachineEnhancements[choice - 1]))
            {
                ...
                string col = (item.isWeapon == true) ? Colour.MONSTER : Colour.SPEAK;
